...
type BottomStepsHandlerProps = {
  ...
  handleUpdateCatalogue: Function
  isUpdating?: boolean
}

const BottomStepsHandler = ({ activeStep, handleActiveStep, handleUpdateCatalogue, isUpdating }: BottomStepsHandlerProps) => {
  return (
    <div className='p-4 rounded-md bg-[#fff] flex justify-end gap-4 items-center'>
      ...
      {
        activeStep == 3 ?
          <Button variant='primary' className='w-[82px] py-2 gap-2 text-sm font-semibold' onClick={() => { handleUpdateCatalogue() }} isLoading={isUpdating}>
            Update
          </Button>
